...
def login_for_access_token(
    ...
    if not user:
        raise HTTPException(
            ...
            detail="Incorrec# /routers/auth.py
from datetime import timedelta
from typing import Annotated
...
def login_for_access_token(
    ...
    FastAPI's OAuth2PasswordRequestForm dependency provides a form with:
    - username: str
    - password: str
    """
    user = services.authenticate_user(
    ...
    return {"access_token": access_token, "token_type": "bearer"}t username or password",
            headers={"WWW-Authenticate": "Bearer"},
        )
