...
class Mission(SQLModel, table=True):
    ...
    # O campo da chave estrangeira DEVE ter um nome diferente do relacionamento.
    createdById: uuid.UUID = Field(foreign_key="user.id")
    createdAt: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    updatedAt: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    isActive: bool = Field(default=True)

    # Este é o relacionamento que usa o `createdById` para encontrar o usuário.
...
class MissionParticipant(SQLModel, table=True):
    ...
    user: "User" = Relationship(back_populates="participations")# /models.py
